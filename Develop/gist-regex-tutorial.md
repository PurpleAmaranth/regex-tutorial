...
Then the $ tags where we want te end of the line to be. So in this case we want the end of the line to be more flexible, since the length of a URL is always variable based on some sort of file structure or pathing. We'll cover more on this soon! Hence we have `/?$/`
...
The OR Operator is denoted by '|'. We can use it to change a range of digits like [abc], which will look for 'abc', into (a|b|c). This will now accept 'abc' as well as'a' or 'b' or 'c'.
